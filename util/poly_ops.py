...
def pad_gt_polys(gt_instances, num_queries_per_poly, device):
    ...
    ##print('DEBUG pad_gt_polys: num_queries_per_poly',num_queries_per_poly)
    
    room_targets = []
    # padding ground truth on-fly
    for gt_inst in gt_instances:#####for each element in the batch
        room_dict = {}
        room_corners = []
        corner_labels = []
        corner_lengths = []

        ##print('DEBUG pad_gt_polys: rooms count',len(gt_inst))

        for i, poly in enumerate(gt_inst.gt_masks.polygons):####for each room in the element
            corners = torch.from_numpy(poly[0]).to(device)

            ##print('DEBUG pad_gt_polys: corners count',corners.shape)            
