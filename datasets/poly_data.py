...
class MultiPoly(Dataset):
    def __init__(self, img_folder, ann_file, transforms, semantic_classes, seg_map = False, metric_data = False):
        super(MultiPoly, self).__init__()

        ##print('debug multiPoly',img_folder, ann_file)
        ...
        self.prepare = ConvertToCocoDict(self.root, self._transforms, seg_map = seg_map, metric_data = metric_data)
...
class ConvertToCocoDict(object):
    def __init__(self, root, augmentations, seg_map = False, metric_data = False):
        self.root = root
    ...
    def __call__(self, img_id, path, target):
        ...
        if self.augmentations is None:
            if(self.seg_map):
                record['image'] = torch.as_tensor(np.ascontiguousarray(np.expand_dims(img, 0)))
            else:
            ...
            if(self.seg_map):
                record['image'] = torch.as_tensor(np.array(np.expand_dims(image, 0)))
            else:
...
def build(image_set, args):
    ...
    ##print('building polygonal dataset from',args.dataset_root)
    ...
    dataset = MultiPoly(img_folder, ann_file, transforms=make_poly_transforms(image_set), semantic_classes=args.semantic_classes, 
                        seg_map = args.seg_image, metric_data = args.save_metric)
